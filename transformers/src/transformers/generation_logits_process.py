...
class MinProbLogitsWarper(LogitsWarper):
    ...
    def __init__(self, min_prob: float, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1):
        if not isinstance(min_prob, float) or min_prob <= 0:
            raise ValueError(f"`top_k` has to be a strictly positive integer, but is {min_prob}")
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        indices_to_remove = scores < np.log(self.min_prob)
        # print(np.log(self.min_prob))
        scores = scores.masked_fill(indices_to_remove, self.filter_value)
        return scores
